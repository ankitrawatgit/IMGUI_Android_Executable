...
bool ImGui::tab1(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(55, 55), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    window->DrawList->AddLine(bb.Max, ImVec2(bb.Min.x, bb.Max.y), ImColor(57, 62, 70), 1.5f);
...
bool ImGui::tab(const char* label, const char* icon, const char* ksub, bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 ksub_size = ImGui::CalcTextSize(ksub, NULL, true);

    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(115, 27), label_size.x*2 + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 0);
    ...
    if (selected)
    {
        ...
        draw->AddRectFilled(bb.Min, ImVec2(bb.Max.x+10, bb.Max.y), ImColor(44, 52, 62), 0,0);
        draw->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + (size.y / 2)), 7, ImColor(101, 107, 114), 360);
        ImGui::PushFont(info_little);
        ImGui::RenderText(ImVec2(bb.Max.x - 13.5f - (ksub_size.x / 2), bb.Min.y + (size.y / 2) - (label_size.y / 2) + 2), ksub);
        ImGui::PopFont();
    }

    ImGui::PushFont(info);
    ImGui::RenderText(ImVec2(bb.Min.x + 30, bb.Min.y + (size.y / 2) - (label_size.y / 2)), label);
    ImGui::PopFont();
    ImGui::PushFont(iconfont);
    ImGui::RenderText(ImVec2(bb.Min.x + 10, bb.Min.y + (size.y / 2) - (icon_size.y / 2)), icon);
    ImGui::PopFont();
...
bool ImGui::sub(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(115, 27), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (selected)
    {
        draw->AddRectFilled(bb.Min, ImVec2(bb.Max.x+20, bb.Max.y), ImColor(44, 52, 62), 0,0);
        
    }
    
     
    ImGui::RenderText(ImVec2(bb.Min.x + 10, bb.Min.y + (size.y / 2 - label_size.y / 2)), label);
...
bool ImGui::da(const char* label, ImFont* font, bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(35, 35), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    {
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(133 / 255.f, 133 / 255.f, 133 / 255.f, 255));
        ImGui::PushFont(font);
        ImGui::RenderText(ImVec2(bb.Min.x + size.x / 2 - label_size.x / 2, bb.Min.y + (size.y / 2 - label_size.y / 2)), label);
        ImGui::PopFont();
        ImGui::PopStyleColor();
...
bool ImGui::InputTextSearch(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(230, 25), flags, callback, user_data);
}
